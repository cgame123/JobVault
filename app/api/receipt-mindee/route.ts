...
import { processReceiptWithMindee } from "@/lib/mindee-client"
...
export async function GET(req: Request) {
  try {
    ...
    console.log("Testing Mindee receipt extraction with image:", imageUrl)
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: "Receipt processed successfully with Mindee",
        data: receiptData,
        imageUsed: imageUrl,
      })
    } catch (error) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Error processing with Mindee",
          error: String(error),
        },
        { status: 500 },
      )
    ...
    console.error("Error in receipt Mindee endpoint:", error)
    return NextResponse.json(
      {
        success: false,
        message: "Error in receipt Mindee endpoint",
        error: String(error),
      },
      { status: 500 },
    )
